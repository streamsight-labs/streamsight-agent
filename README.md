...
`DESCRIBE_CONFIGS` on `TOPIC` and `CLUSTER`. All five are read-only.
...
[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#request-budget-per-cycle).

**Status: unreleased.** No `v*` tag has been cut, so `:latest` and the version tags do not
resolve yet — deploy `:main` or `sha-<short>`.
...
One cycle runs seventeen phases. Five of them sample the offset chain, and the order in which
they are sampled is a correctness constraint rather than an implementation detail — see
[Collection order](#collection-order-is-a-correctness-constraint). The rest either wait for
that chain to finish, sample on their own cadence, fire only on a trigger, or issue no
request at all. [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) has the package map, the
per-cycle sequence and the request budget.

## What it collects

Per cycle, from the Admin API only. The optional collectors are one line each here; their
cost, version floor and trade-off are on their row under [Configuration](#configuration).
...
* **Log directories** (`COLLECT_LOG_DIRS`, on) — per-replica on-disk bytes, per-replica
  offset lag, in-flight JBOD moves, offline disks, and the volume's total and usable bytes.
  See [Log directories](#log-directories).
* **Max timestamp** (`COLLECT_MAX_TIMESTAMP`, on) — the newest record's timestamp and the
  offset carrying it: topic liveness measured rather than inferred.
* **Tiered offsets** (`COLLECT_TIERED_OFFSETS`, opt-in) — the local log start, and
  optionally the remote end.
* **Topic and broker configs** (`COLLECT_CONFIGS`, on) — a fixed allowlist of 10 topic and
  11 broker keys.
* **Share groups** (`COLLECT_SHARE_GROUPS`, opt-in, Kafka 4.0+) — KIP-932 queue-semantics
  groups, in their own section.
* **Throughput window** (`COLLECT_THROUGHPUT_WINDOW`, opt-in) — a produce rate measured by
  the broker rather than differenced across batches.
...
* **Capabilities** — what the cluster can be asked, per broker: the API versions behind most
  of the gates above — `list_offsets`, `metadata`, `list_groups`, `offset_for_leader_epoch`,
  `describe_log_dirs`, `list_partition_reassignments`, `consumer_group_describe` — so an
  absent signal is attributable to a broker version rather than to collection breaking.
  `DescribeConfigs` and `ShareGroupDescribe` are probed to gate their sections but are not
  reported here, so a `skipped` `topic_configs`, `broker_configs` or `share_groups` is not
  attributable from this section.
...
saw them, which is a property of one client's connection to a broker, not of the broker.)
...
The five phases that sample the offset chain are sampled in a fixed order, and that order
decides the *sign* of the error you get from the unavoidable skew between the calls. A
caught-up consumer must never report `-3`, and a hung transaction must never report a
negative backlog and read as healthy:
...
offsets or that count comes out negative.

The *wire* order of `sections[]` is a different fixed order, listed under
[Reading the data correctly](#reading-the-data-correctly). The full phase sequence, which
phases wait on which, and why, are in [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
`selection` answers "is this the whole cluster?" — it is what lets the far end tell "the
cluster has 40 topics" from "the agent was pointed at 40 of them". It echoes the filters in
force —
...
* **`groups[].generation` is `-1` in practice, and is not a rebalance counter.** Kafka's
  ...
  member-ID churn in `groups[].members[].member_id` — the latter is the measured one: against
  a real rebalance storm it caught all 31 member-set changes where sampling group state caught
  2, since a rebalance completes well inside a poll interval
  ([docs/TESTING.md](docs/TESTING.md#5-multi-broker-under-load-and-failure-khaos)).
...
* `broker_rpc` histograms are per broker per API key with no cap of their own, and there is
  no switch for them: an agent that cannot report its own request latency cannot be
  supported. Their measured share of a batch is under [Output](#output).
...
| `COLLECTION_TIMEOUT` | 80% of `COLLECTION_INTERVAL` (`4s`) | Per-cycle deadline. Exceeding the interval is a startup warning, not an error. A cycle that outruns kgo's metadata cache — half the interval, 2.5s at the defaults — resolves its later offset listings against a fresher topic set than the `cluster` section shipped; see [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#request-budget-per-cycle). |
...
| `TOPIC_EXCLUDE` | `""` | Exclude wins over include: a topic matching both is dropped. `TOPIC_EXCLUDE=__consumer_offsets` drops that one topic; `TOPIC_EXCLUDE=/-dlq$/,/^tmp-/` drops the two shapes. Because a literal entry is escaped, excluding `orders.events` does not take `ordersXevents` with it. |
...
All four name filters share that form. The literal is the default because Kafka names are
full of dots and `.` is a regex metacharacter: escaping and anchoring every literal is what
makes `TOPIC_INCLUDE=orders.events,payments.v2` select two topics rather than a family of
them. Reach for the slashes when you want a pattern, and put in the `^` and `$` yourself —
what sits between them is compiled exactly as written. The one thing a slash-wrapped entry
cannot contain is a comma, which the list splitter claims before the pattern compiler ever
sees it; `/^(a|b)$/` is the way to write the alternation a `,` would have meant. Group IDs
beginning with `__` are always skipped.
...
no ACL, no broker work. The first two are still switched off automatically on a cluster too
old to serve them, which is a capability probe's job rather than an operator's. Nothing
gates `agent.rpc`: it needs no broker support, so it is always on.
...
filtered set is indistinguishable at the backend from a deleted one: the broker never lists
it, so there is nothing for the agent to mark as truncated.
...
read these as the backstop against an error storm from a cluster that is misbehaving. Both ends of the
range log a startup warning: `MAX_ERRORS=0`, because it removes a ceiling rather than
choosing one, and `MAX_ERROR_SAMPLES` above 50, because that many verbatim occurrences
largely defeats the deduplication the cap sits behind.
...
| `MAX_ERROR_SAMPLES` | `1` | Verbatim occurrences emitted per distinct failure mode before the rest are folded into the exemplar's `count`. Must be >= 1; above 50 logs a warning. |
...
those terms. Say what a deployment does not want to watch in the filters above instead:
they shrink the same broker requests a cap would have, and travel with the data in
`selection`.
...
when `EXPORT_ENDPOINT` is set and `file` otherwise.
...
* One batch may occupy the single export worker for at most **one minute** of waiting: the
  budget is checked before each backoff, so a batch that cannot fit its next delay is abandoned
  rather than sleeping past it. An attempt already in flight is not cut short — it stays bounded
  by `EXPORT_TIMEOUT` — so worst-case occupancy is one minute plus one `EXPORT_TIMEOUT`, 70s at
  the defaults. Without that ceiling three retries each honouring a clamped `Retry-After: 300`
  would hold the worker for fifteen minutes while the queue behind it turned over completely and
  the agent shipped nothing.
...
|----------|-----------|---------|
| `CLUSTER` | `DESCRIBE` | `Metadata`, `ListGroups`, `DescribeLogDirs`, `ListPartitionReassignments` |
| `TOPIC` | `DESCRIBE` | Topic and partition metadata; every `ListOffsets` flavour; `OffsetForLeaderEpoch`; the topics inside `OffsetFetch` |
| `GROUP` | `DESCRIBE` | `DescribeGroups`, `ConsumerGroupDescribe`, `OffsetFetch`, and the two share-group APIs |
| `TOPIC` | `DESCRIBE_CONFIGS` | `DescribeConfigs` for `topic_configs[]` |
| `CLUSTER` | `DESCRIBE_CONFIGS` | `DescribeConfigs` for `broker_configs[]` |

That is the complete set. `ApiVersions` (the startup probes) is answered before
authentication completes and carries no ACL at all. `DESCRIBE_CONFIGS` is read-only and is
**not** `ALTER_CONFIGS`: it permits reading configuration, never changing it.

Because `COLLECT_LOG_DIRS` defaults on, the `CLUSTER` grant is load-bearing for a data
section by default and not only for cluster metadata.

The three `DESCRIBE` grants are verified end to end against a broker with
`allow.everyone.if.no.acl.found=false` holding exactly that set, each `CLUSTER`-gated API
with its own negative control; removing any one grant degrades the corresponding section to
`unauthorized` rather than silently emptying it. The two `DESCRIBE_CONFIGS` rows are the
ones still read from the broker's authorization rules rather than exercised — what the local
rig measures is the refusal. [docs/TESTING.md](docs/TESTING.md#3-the-permission-model-sasl--acls)
has the procedure and the negative controls;
[SECURITY.md](SECURITY.md) states the same grants as a product invariant, with what the
agent sends where and what a batch does and does not contain — that is the file to hand to
a security team.
...
Grant the ACLs with `kafka-acls.sh`, exactly as on a self-managed cluster.
...
`.github/workflows/promote.yml` after a `v*` release tag.
...
the chart defaults to `.Chart.AppVersion`, the manifests pin `:latest` — so pass
`--set image.tag=main` to `helm install`, or `kubectl set image` after applying the
manifests.
...
[docs/TESTING.md](docs/TESTING.md) is the full guide: six levels, from unit tests up to a
multi-broker cluster under induced failure. The two you will reach for first:

```bash
docker compose up -d    # one broker, no auth, agent in file mode
make test-local-up      # SASL/SCRAM + ACLs, the permission model the product sells
```

`make test-local-up` provisions the agent's credential and ACLs exactly as
`scripts/setup-kafka-user.sh` would, seeds a topic and a lagging consumer group, and runs
the agent under nothing but the three `DESCRIBE` grants — so revoking one and watching a
single section turn `unauthorized` is a one-command experiment. See
[docs/TESTING.md](docs/TESTING.md#3-the-permission-model-sasl--acls) for that procedure,
[level 4](docs/TESTING.md#4-http-export-against-the-conformance-mock) for the `http` export
path against the conformance mock, and
[level 6](docs/TESTING.md#6-kafka-4x-the-two-group-protocols-no-older-broker-can-serve) for
the KIP-848 and KIP-932 paths every older broker gates off.
...
make check        # gofmt check, go vet, go test — the pre-push gate
```

The test, lint and coverage targets are listed in
[docs/TESTING.md](docs/TESTING.md#1-unit-tests).
...
The agent is statically linked, so a release binary and the container image each
redistribute every dependency as well as the agent itself. All of those dependencies are
BSD-3-Clause, whose binary clause asks that the copyright notices travel with the
distribution rather than only with the source, so
[THIRD_PARTY_NOTICES.txt](THIRD_PARTY_NOTICES.txt) is attached to every release and copied
into the image at `/usr/local/share/streamsight-agent`.
